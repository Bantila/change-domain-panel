...
7. **Проверяет `nginx.conf` до перезапуска** — гоняет `nginx -t` в одноразовом
   контейнере с тем же образом, что и в проекте. Битый конфиг видно до того, как
   упадёт боевой контейнер.
8. **Перезапускает** нужные контейнеры (в docs.rw — все затронутые compose-проекты).
   Для ноды сначала поднимается reverse-proxy, и только после того, как он занял
   свои порты, стартует `remnanode` — иначе они дерутся за порт.
9. **Меняет домен в самой панели по API**: Config Profile (`serverNames`, `dest`,
   `host`), Hosts (`address`, `sni`, `host`) и адрес ноды (Nodes → `address`).
10. **Проверяет результат**: HTTP-код нового домена, статус контейнеров и то, что
    они не крутятся в restart-loop. Если что-то не поднялось — показывает логи и
    предлагает откатиться из бэкапа.
...
| `--no-node-restart` | нет | Не перезапускать ноды после правки Config Profile. |
...
По формату самой строки:

| Ключ | Формат | Что пишется в `cloudflare.ini` |
|---|---|---|
| Global API Key | 37 символов, hex в нижнем регистре | `dns_cloudflare_email` + `dns_cloudflare_api_key` |
| API Token | 40 символов из `A-Za-z0-9_-` | `dns_cloudflare_api_token` |

Если строка не подходит ни под один формат, скрипт спрашивает тип явно, а не
угадывает.
...
| Перезапуск нод | `POST /api/nodes/{uuid}/actions/restart` | — |
...
- **Ноды перезапускаются после правки профиля.** Сам по себе изменённый Config
  Profile до ноды не доезжает: конфиг она получает от панели при рестарте. Иначе в
  панели уже новый домен, а нода до ближайшего перезапуска продолжает отдавать
  старый `serverNames`. Перезапускаются только ноды, сидящие на изменённых
  профилях, и те, которым поменяли адрес — чужие не трогаются. Отключается флагом
  `--no-node-restart`.
...
- Проверка `nginx -t` перед перезапуском может забраковать рабочий конфиг —
  например, если он подключает внешние файлы через `include`, которых нет в
  одноразовом контейнере. Поэтому её вердикт не финальный: скрипт показывает
  вывод и спрашивает, перезапускать ли всё равно.
...
**`bind() to 0.0.0.0:8443 failed: Address already in use` у nginx на ноде**
Xray-инбаунд ноды и decoy-сервер оба слушают внешний интерфейс на одном порту
(`network_mode: host`), и при одновременном старте выигрывает тот, кто успел.
Скрипт разводит их по времени — сначала reverse-proxy, потом `remnanode`, — но
сама конфигурация при этом остаётся конфликтной.

Архитектурно правильнее, чтобы decoy слушал `127.0.0.1:<порт>`, а не
`0.0.0.0:<порт>`: Reality сам проксирует на него нераспознанный трафик через
localhost, и порты физически перестают пересекаться на внешнем интерфейсе. Это
правится в конфиге ноды, скрипт туда не лезет.

**Контейнер поднялся и сразу упал**
Скрипт это замечает (следит за ростом `RestartCount`), показывает последние
строки лога и предлагает откатить файлы из бэкапа. Если отказался от отката —
он всегда доступен потом: `changedomain --rollback <каталог бэкапа>`.
...
Скрипт целиком проверяется только на сервере, но чистые функции (замена домена,
пути к сертификату, поиск cookie панели, распознавание ключа Cloudflare) покрыты
проверками на временных фикстурах — без root, docker и сети:

```bash
bash test_change_domain.sh
```

Функции для проверки берутся из самого `change-domain.sh`, так что тест ходит по
реальному коду, а не по его копии.
